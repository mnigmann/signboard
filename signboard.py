...
root = "/home/matthias/Documents/Python"
...
def load(root):
  ...
  with open(root+ "/neopixel/structure.json") as struc: 
    struc = json.load(struc)
    with open(root+"/neopixel/"+struc['settings']['alphabet']) as l: 
      l = json.load(l)
      letters = l["letters"]
      ...
      images = {obj["path"]: load_img(Image.open(root+"/neopixel/images/"+obj["path"])) for obj in objects if obj["type"]=="image"}
      images.update({p['path']: load_img(Image.open(root+"/neopixel/images/"+p['path'])) for obj in objects if obj['type']=='animation' for p in obj['frames']})
      ...
      for x in letters_scaled['a']: print(x)
      ...
      for i, object in enumerate(objects):
        ...
        while True:
            currFrame = [object['background']]*LENGTH
            for ci, c in enumerate(object['phrase']):
                ...
                for x in range(sHeight):                            #render a letter
                    for y in range(sWidth):
                        setPixel(ROWS-sHeight+x, y+offset, object['color'] if l[x][y] else object['background'], currFrame)     #Set a pixel
            index += object['step']
            if index >= len(object["phrase"])*scale*(WIDTH+1)+COLS:
                break
            phrases_rendered[i].append(currFrame)
...
def save_objects():
  global struc
  with open(root+"/neopixel/structure.json", "w") as f:
